...
- We build admin page based on https://github.com/akveo/ng2-admin

    1. Just build anywhere ( not under backend folder )
    
    2. Publish on the same folder of http://backend.org/admin
...
## How to do "Integer Comparison".

Since all value in `bind` is being escaped as string, it is like you cannot do number comparison.

The trick is to use `CAST` and faking bind marks like below.


````
    this.searchQuery.order = 'idx DESC';
    this.searchQuery.where = "deleted is null and cast(? as integer)";  // We need to put at least one question mark(?).
    this.searchQuery.bind  = '1';                                       // and the value must be a digit here to make it TRUE.
````
